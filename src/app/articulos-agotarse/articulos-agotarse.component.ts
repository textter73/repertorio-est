...
@Component({
  selector: 'app-articulos-agotarse',
  templateUrl: './articulos-agotarse.component.html',
  styleUrls: ['./articulos-agotarse.component.scss']
})
export class ArtículosAgotarseComponent implements OnInit {
    ...
    getCatalog(): void {
      ...
      this._catalogoServices.getCatalogV2().subscribe((listItems) => {
        ...
        for (const item of this.itemsList) {
            if (+item.cantidad <= 4 && item.activo === true && item.comprarDespues === true) {
                temporal.push(item);
            }
        }
